...
export class BlockscoutPageManager extends AbstractPageManager {
    async init() {
        this.check();
    ...
    findPlacesForReverseResolve() {
        return new Promise((resolve) => {
            ...
            const selector = 'div.chakra-skeleton.css-1roy00r, a[href^="/address/"].chakra-link.css-1yfgo13';
            ...
            const observer = new IntersectionObserver((entries) => {
                for (const entry of entries) {
                    if (entry.isIntersecting) {
                        ...
                        if (!element.href && element.querySelector(selector)) continue; // has another element inside
                        let address;
