...
export class Tipping {
    constructor(identifier) {
        ...
        this.div.shadowRoot.append(template({identifier, networks, tokens, eth_logo, usdc_logo, arrow, pen, close}));

        this.div.shadowRoot.querySelectorAll('.select').forEach(select => {
            select.onclick = e => select.classList.toggle('isOpen')
            select.onblur = e => select.classList.remove('isOpen')
        })
        this.div.shadowRoot.querySelectorAll('.select ul li').forEach(li => {
            li.onclick = e => {
                e.stopPropagation();
    ...
    refreshVisibleCoins() {
        let chainId = this.div.shadowRoot.querySelector('.networkSelect').dataset.chainId;
        let tokens = this.div.shadowRoot.querySelectorAll('.tokenSelect li')
        for (let token of tokens) {
            token.style.display = token.dataset.chainId == chainId ? '' : 'none';
        }
        if (this.div.shadowRoot.querySelector('.tokenSelect').dataset.chainId != chainId) {
            this.div.shadowRoot.querySelector(`.tokenSelect li[data-chain-id="${chainId}"]`).click();
        }
    }
}
