...
export class AbstractPageManager {
  ...
  generatePopupContent(div, key, elements, callback) {
    ...
    if (Object.values(elements).length > 3) {
      addition =
        ':host {max-height: 125px!important; overflow-y: auto!important; overflow-x: hidden!important;}';
    }
  ...
  apiCall(value) {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage(
        { type: 'apiAddressesRequest', value },
        (response) => {
          resolve(response);
        },
      );
  ...
  reverseResolveRequest(value) {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage(
        { type: 'reverseResolveRequest', value },
        (response) => {
          resolve(response);
        },
      );
  ...
  async findReverseResolve() {
    ...
    let addresses = places
      .map((x) => x.address)
      .filter((a) => this.reverseKnownAddresses[a] === undefined);
