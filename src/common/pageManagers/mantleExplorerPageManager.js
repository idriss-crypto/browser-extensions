...
export class MantleExplorerPageManager extends AbstractPageManager {
    async init() {
        this.check();
    ...
    findPlacesForReverseResolve() {
        return new Promise((resolve) => {
            ...
            const selector = ".address-detail-hash-title, [data-address-hash]:not(.contract-address)";
            const spans = document.querySelectorAll(selector);

            const ethereumAddressSpans = Array.from(spans).filter((span) => {
                let ethereumAddress = span.textContent.trim();
                let isEthereumAddress = /^0x[a-fA-F0-9]{40}$/.test(ethereumAddress);
                if (!isEthereumAddress) {
                    ethereumAddress = span.getAttribute('data-address-hash');
                    isEthereumAddress = /^0x[a-fA-F0-9]{40}$/.test(ethereumAddress);
                }
            ...
            const observer = new IntersectionObserver((entries) => {
                for (const entry of entries) {
                    if (entry.isIntersecting) {
                        ...
                        let address;
                        if (!element.href) {
                            let addressTemp = element.textContent;
