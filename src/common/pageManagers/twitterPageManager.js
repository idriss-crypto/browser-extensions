...
export class TwitterPageManager extends AbstractPageManager {
  ...
  async check() {
    ...
    if (await this.isEnabled()) {
      ...
      Array.from(document.querySelectorAll(".idrissIcon")).forEach((x) =>
        x.remove()
      );
    }
  ...
  checkGarbageDropdown() {
    const selector =
      ".idrissIcon:focus, .idrissIcon:hover, .idrissTwitterDropdown:hover, .idrissTwitterDropdown:focus, .idrissTwitterDropdown.isClicked";
  ...
  async getInfo(names) {
    const lacking = Array.from(names).filter(
      (x) => !TwitterPageManager.namesResults[x]
    );
    ...
    for (const name of lacking) {
      TwitterPageManager.namesResults[name] = requestPromise.then(
        (d) => d[name]
      );
    }
  ...
  *listPlaces() {
    for (const div of document.querySelectorAll(
      ...
      const name = Array.from(
        div.querySelectorAll(".r-9ilb82, .r-14j79pv, .r-rjixqe")
      )
        .map((x) => x.textContent)
        .find((x) => x[0] == "@");
      if (!name) continue;
      ...
      const addCallback = (data) => {
        if (!data.error && !div.querySelector(".idrissIcon")) {
          if (Object.values(data).length === 0) {
            ...
            const dropdownContent = new Tipping(data).div;
