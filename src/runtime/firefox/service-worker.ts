...
*/
  ...
  browser.tabs.onUpdated.addListener((tabId, _, tab) => {
    const isActive =
      Boolean(tab.url?.includes('f8447b2f-6dfc-45ef-acea-dbe7d9828659')) ||
      Boolean(tab.url?.includes('92744c8c-9eb2-4c22-8d0c-f0fb8a0bb9ca')) ||
      Boolean(tab.url?.includes('b40d9360-a5a5-43e5-928d-cbe6a73b66a1')) ||
      Boolean(tab.url?.includes('0b8be570-8dcc-4755-95b4-bb5da48fa031')) ||
      Boolean(tab.url?.includes('b7f2adfa-eee4-4e2e-a1dc-b6264f24087b)'));
...
ServiceWorker.run(browser, onInstalled);
