...
export const TradingCopilotSettingsManager = {
  getAllSubscriptions(): Promise<Subscription[]> {
    return chrome.storage.local
      ...
      .then((result) => { return result[SUBSCRIPTIONS_STORAGE_KEY] as Subscription[] ?? [] });
  },

  async subscribe(subscription: Subscription): Promise<void> {
    const allSubscriptions = await this.getAllSubscriptions();

    if (allSubscriptions.some((existingSubscription) => { return existingSubscription.ensName === subscription.ensName })) {
  ...
  async unsubscribe(ensName: Subscription['ensName']): Promise<void> {
    ...
    return chrome.storage.local.set({
      [SUBSCRIPTIONS_STORAGE_KEY]: allSubscriptions.filter(existingSubscription => { return existingSubscription.ensName !== ensName }),
    });
  },
};