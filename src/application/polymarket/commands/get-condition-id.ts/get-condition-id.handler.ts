...
export class GetConditionIdHandler implements Handler {
  async handle(command: GetConditionIdCommand) {
    ...
    if (!url) {
      return new FailureResult('Document does not contain polymarket event');
    }
    const polymarketResponse = await fetch(url);
    const polymarketHtml = await polymarketResponse.text();
