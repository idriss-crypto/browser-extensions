    ...
    return chrome.storage.local
      ...
      .then((result) => {
        const allSubscriptions =
          (result[SUBSCRIPTIONS_STORAGE_KEY] as Subscription[]) ?? [];

        if (
          allSubscriptions.some((sub) => {
            return sub.ensName === subscription.ensName;
          })
        ) {
  ...
  unsubscribe(subscription: Subscription) {
    return chrome.storage.local
      ...
      .then((result) => {
        const allSubscriptions =
          (result[SUBSCRIPTIONS_STORAGE_KEY] as Subscription[]) ?? [];
        const subscriptionIndex = allSubscriptions.findIndex((sub) => {
          return sub.ensName === subscription.ensName;
        });
